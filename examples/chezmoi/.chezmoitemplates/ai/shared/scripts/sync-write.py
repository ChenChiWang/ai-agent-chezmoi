...
SCRIPTS = ('sync.sh', 'scan-secrets.py', 'sync-write.py', 'gitleaks-rules.json')
LIMIT = api.MAX_BYTES
...
class Engine:
    def __init__(self, args, tmp):
        ...
        for name in ('MERGE_HEAD', 'CHERRY_PICK_HEAD', 'REVERT_HEAD', 'rebase-merge', 'rebase-apply',
                     'BISECT_LOG', 'index.lock', 'ai-agent-sync-transaction'):
            need(not (self.gd / name).exists(), 'BLOCKED_CONFLICT', 66)
        self.remote = self.remote_url(args.remote)
    ...
    def render(self, snap):
        # Literal fixed schema, identical to chezmoi include output (integration tested).
        opening, closing = '{' * 2, '}' * 2
        include = lambda p, trim: (opening + ' include "' + p + '" ' + ('-' if trim else '') + closing + '\n').encode()
        wrappers, result = {}, {}
        for product, target in [('claude', '.claude/CLAUDE.md'), ('codex', '.codex/AGENTS.md')]:
            banner = b'<!-- Generated by chezmoi. Edit .chezmoitemplates/ai/ in the source. -->\n'
            wrappers['dot_' + product + '/' + target.split('/')[-1] + '.tmpl'] = (
                banner + include(PREFIX + 'shared/instructions.md', False)
                + include(PREFIX + 'adapters/' + product + '.md', True))
            result[target] = (banner + snap[PREFIX + 'shared/instructions.md'][0] + b'\n'
                              + snap[PREFIX + 'adapters/' + product + '.md'][0], 0o644)
        for product, target in [('claude', '.claude/skills/dotfiles-sync/SKILL.md'),
                                ('agents', '.agents/skills/dotfiles-sync/SKILL.md')]:
            wrappers['dot_' + product + '/skills/dotfiles-sync/SKILL.md.tmpl'] = include(
                PREFIX + 'shared/skills/dotfiles-sync/SKILL.md', True)
            result[target] = (snap[PREFIX + 'shared/skills/dotfiles-sync/SKILL.md'][0], 0o644)
        for name in SCRIPTS:
            executable = name != 'gitleaks-rules.json'
            wrappers['dot_config/ai-agent/bin/' + ('executable_' if executable else '') + name + '.tmpl'] = include(
                PREFIX + 'shared/scripts/' + name, True)
            result['.config/ai-agent/bin/' + name] = (snap[PREFIX + 'shared/scripts/' + name][0],
                                                   0o755 if executable else 0o644)
        for p, (content, _) in snap.items():
            text_file(content)
            if p in wrappers:
                need(content == wrappers[p], 'UNSUPPORTED_TEMPLATE')
            elif p.startswith('.chezmoitemplates/'):
                need(opening.encode() not in content, 'UNSUPPORTED_TEMPLATE')
        return result
    ...
    def history(self, older, newer):
        ...
        for oid in commits:
            ...
            need(changed and changed <= {p.encode() for p in FILES}, 'BLOCKED_OUT_OF_SCOPE_HISTORY', 66)
    ...
    def state(self):
        return dict(head=self.source_git('rev-parse', 'HEAD').decode().strip(),
                    ...
                    source=summary(snapshot(self.src, FILES)),
                    target=summary(snapshot(self.dst, TARGETS)))
    ...
    def build(self):
        ...
        self.working = snapshot(self.src, FILES)
        self.deployed = snapshot(self.dst, TARGETS)  # Missing targets require separate migration.
        baseline = self.scoped(self.head)
        self.scan(baseline)
        self.scan(self.working)
        self.scan(self.deployed, 'target')
        base_render = self.render(baseline)
        self.remote_head = self.fetch()
        ...
        return dict(schema=2, operation=self.a.operation, source=str(self.src), destination=str(self.dst),
...
def arguments():
    ...
    for arg in ('source', 'destination', 'remote', 'branch', 'plan'):
    ...
    for value in (a.source, a.destination, a.plan, a.scanner or '/'):
